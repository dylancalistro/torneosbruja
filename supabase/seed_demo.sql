...
begin
  ...
  insert into partidos (torneo_id, equipo_local_id, equipo_visitante_id, goles_local, goles_visitante, fecha, jugado)
  values (v_torneo_id, v_equipo_norte, v_equipo_sur, 2, 1, now() - interval '7 days', true)
  returning id into v_partido1;

  insert into partidos (torneo_id, equipo_local_id, equipo_visitante_id, goles_local, goles_visitante, fecha, jugado)
  values (v_torneo_id, v_equipo_central, v_equipo_rio, 1, 1, now() - interval '5 days', true)
  returning id into v_partido2;

  insert into partidos (torneo_id, equipo_local_id, equipo_visitante_id, fecha, jugado)
  values (v_torneo_id, v_equipo_norte, v_equipo_central, now() + interval '3 days', false);
